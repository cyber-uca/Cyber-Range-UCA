...
const DIFF_COLOR = { beginner:'var(--green)', intermediate:'var(--amber)', advanced:'var(--red)', expert:'var(--red)', easy:'var(--green)', medium:'var(--amber)', hard:'var(--red)' }
  ...
  return (
    <div style={{ flex:1, minWidth:200, maxWidth:290,
      ...
      <div style={{ display:'flex', justifyContent:'center', marginTop:8 }}>
        <div style={{ width:10, height:10, borderRadius:'50%', border:`1px solid ${meta.color}50`,
...
export default function Roadmap() {
  ...
  useEffect(() => {
    // Fetch each path's full detail (includes modules + rooms)
    api.listPaths().then(async (cards) => {
      const full = await Promise.all(cards.map(c => api.getPath(c.slug)))
      setPaths(full)
    ...
    <div className="page page-full fade-up" style={{ padding:'40px 44px' }}>
      ...
      <div style={{ display:'flex', gap:14, alignItems:'flex-start', overflowX:'auto', paddingBottom:32 }}>
        {sorted.map(pathData => {
          ...
          return (
            <PathCol key={pathData.slug} pathData={pathData} meta={meta}
