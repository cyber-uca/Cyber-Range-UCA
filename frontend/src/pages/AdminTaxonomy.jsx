...
const C_MAP  = { coral:'var(--cat-offensive)', blue:'var(--cat-defensive)', teal:'var(--cat-mitigation)', purple:'var(--cat-risk)' }
  ...
  return (
    <div className="page fade-up" style={{ paddingTop: 0 }}>
      ...
      <div className="ad-header">
        ...
        <div>
          <div className="ad-eyebrow">Admin · Taxonomy</div>
          <h1 className="ad-title">Categories &amp; Difficulties</h1>
          <p className="ad-subtitle">Changes appear everywhere instantly — no redeploy needed.</p>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }}>
        {/* Categories */}
        <div>
          <div className="card fade-up-1" style={{ padding:0, overflow:'hidden', marginBottom:16 }}>
            <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <h3 style={{ margin:0, fontSize:14 }}>Categories</h3>
              <span style={{ fontSize:12, color:'var(--text-4)' }}>{categories.length}</span>
            </div>
            {categories.length===0
              ? <div style={{ padding:20, color:'var(--text-4)', fontSize:13 }}>No categories yet.</div>
              : <table className="data-table">
                  <tbody>
                    {categories.map(c => (
                      <tr key={c.id}>
                        <td><span className={`badge badge-${c.slug}`}>{c.name}</span></td>
                        <td style={{ color:'var(--text-4)', fontSize:12, fontFamily:'var(--mono)' }}>{c.slug}</td>
          ...
          <div className="form-section fade-up-2">
            <h3 style={{ color:'var(--text-2)' }}>Add a category</h3>
                  ...
                  {COLORS.map(c => (
                    <div key={c} onClick={() => setCatForm({...catForm,color:c})}
                      style={{ padding:'5px 12px', borderRadius:999, fontSize:12, cursor:'pointer',
                        background: catForm.color===c ? `${C_MAP[c]}18` : 'var(--surface-3)',
                        color:C_MAP[c], border:`1px solid ${catForm.color===c ? C_MAP[c]+'60':'var(--border)'}`,
        ...
        <div>
          <div className="card fade-up-1" style={{ padding:0, overflow:'hidden', marginBottom:16 }}>
            <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <h3 style={{ margin:0, fontSize:14 }}>Difficulties</h3>
              <span style={{ fontSize:12, color:'var(--text-4)' }}>{difficulties.length}</span>
            </div>
            {difficulties.length===0
              ? <div style={{ padding:20, color:'var(--text-4)', fontSize:13 }}>No difficulties yet.</div>
              : <table className="data-table">
                  <tbody>
                    {difficulties.map(d => (
                      <tr key={d.id}>
                        <td style={{ fontWeight:600, color:'var(--text-2)' }}>{d.name}</td>
                        <td style={{ color:'var(--text-4)', fontSize:12, fontFamily:'var(--mono)' }}>order {d.sort_order}</td>
          ...
          <div className="form-section fade-up-2">
            <h3 style={{ color:'var(--text-2)' }}>Add a difficulty</h3>
