    ...
    <div className="page fade-up">
      <div className="page-header">
        ...
        <p className="lead" style={{ marginTop:6 }}>
          Golden images cloned per learner session. Set the Proxmox VMID to match your actual templates.
        </p>
      </div>
      ...
      <div className="card" style={{ padding:0, overflow:'hidden', marginBottom:20 }}>
        {templates.length === 0
          ? <div style={{ padding:'40px', textAlign:'center', color:'var(--text-muted)' }}>No VM templates yet. Add one below.</div>
          : <table className="data-table">
              ...
              <tbody>
                {templates.map(t => (
                  <tr key={t.id} style={{ background: editingId===t.id ? 'rgba(56,189,248,0.03)' : 'transparent' }}>
                    <td>
                      <div style={{ fontWeight:600 }}>{t.name}</div>
                      {t.description && <div style={{ fontSize:11, color:'var(--text-dim)', marginTop:2 }}>{t.description}</div>}
                    </td>
                    <td>
                      <span style={{ fontFamily:'var(--mono)', fontSize:11, background:'var(--surface-2)',
                        padding:'2px 8px', borderRadius:4, border:'1px solid var(--border)', color:'var(--text-muted)' }}>
                        {t.zone}
                      </span>
                    </td>
                    <td>
                      <span style={{ fontFamily:'var(--mono)', fontSize:14, fontWeight:800, color:'var(--accent)' }}>
                    ...
                    <td>
                      <div style={{ display:'flex', gap:6, justifyContent:'flex-end' }}>
                        <button className="btn-secondary btn-sm"
                          style={{ color: editingId===t.id ? 'var(--accent)' : undefined }}
      ...
      <div id="vm-form" className="form-section">
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18 }}>
          <h3 style={{ margin:0 }}>{editingId ? 'Edit VM template' : 'Add a VM template'}</h3>
          {editingId && <button className="btn-ghost btn-sm" onClick={cancelEdit}>Cancel edit</button>}
        </div>
        <form onSubmit={submit}>
          <div className="form-grid">
            <div className="form-row">
              <label>Name</label>
              <input value={form.name} onChange={upd('name')} required placeholder="e.g. icsimrisk" />
            </div>
            <div className="form-row">
              <label>Zone</label>
              <input value={form.zone} onChange={upd('zone')} placeholder="CAN_Net" required />
            </div>
            <div className="form-row">
              ...
              <div style={{ fontSize:11, color:'var(--text-dim)', marginTop:4 }}>Must match a stopped VM or template in Proxmox</div>
            </div>
            <div className="form-row">
              <label>Default tools</label>
              <input value={form.default_tools} onChange={upd('default_tools')} placeholder="nmap, wireshark" />
            </div>
          </div>
          <div className="form-row">
            <label>Description</label>
            <textarea rows={2} value={form.description} onChange={upd('description')} placeholder="Brief description…" />
          </div>
